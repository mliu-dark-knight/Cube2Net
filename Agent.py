...
class Agent(object):
	def __init(self, params, environment):
		self.params = params
		self.environment = environment
		self.build()

	def build(self):
		self.state = tf.placeholder(tf.float32, [None, 2 * self.params.embed_dim])
		...
		for name in self.weights:
			self.assign[name] = tf.assign(self.weights_t[name], self.weights[name])
			...
			for _ in tqdm(range(self.params.epoch), ncols=100):
				...
				for _ in tqdm(range(self.params.k_step), ncols=100):
					...
					for sample in self.environment.sample():
						...
						feed_q.append(r + np.max(sess.run(self.q_t, feed_dict={self.state: state_embeds})))
					sess.run(self.step, feed_dict={self.state: feed_state, self.q_v: feed_q})
